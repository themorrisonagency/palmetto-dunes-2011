...
Galleria.addTheme({
    ...
    defaults: {
        transition: 'none',
        thumbCrop:  true,
		_client_name: 'Palmetto Dunes Oceanfront Resort'
    ...
    init: function(options) {
        ...
        this.bind('loadfinish', function(e) {
			...
			var current_title = encodeURIComponent(this.getData( e.index ).title),
				...
				video_id = encodeURIComponent(this.getData(e.index).video),
				current_media = encodeURIComponent(this.getData(e.index).image);

				/*if(current_desc.length && current_desc !== 'null') {
					full_desc = current_title + ' ' + current_desc + ' Via ' + options._client_name;
				} else {
					full_desc = current_title + ' Via ' + options._client_name;
				}*/
				if(current_desc.length && current_desc !== 'null') {
					full_desc = current_desc + ' Via ' + options._client_name;
				} else {
					full_desc = ' Via ' + options._client_name;
				}
			...
			if(video_id.length && video_id !== 'undefined') {
				//console.log('undefined?');
				$('.fullscreen .gallery-facebook').attr('href','http://www.facebook.com/sharer.php?s=100&v=1&p[title]='+encodeURIComponent(current_title)+'&p[url]=http://youtu.be/'+video_id+'&p[images][0]='+current_media+'&p[summary]='+full_desc+' ('+current_url+')');
			} else {
				$('.fullscreen .gallery-facebook').attr('href','http://www.facebook.com/sharer.php?s=100&v=1&p[images][0]='+current_media+'&p[title]='+current_title+'&p[url]='+current_media+'&p[summary]='+full_desc);
			}

			$('.fullscreen .gallery-twitter').attr('href','http://twitter.com/share?text=' + full_desc +'&url='+ current_media);
			$('.fullscreen .gallery-pinterest').attr('href','http://pinterest.com/pin/create/button/?url='+current_media+'&media='+current_media+'&description='+full_desc);
...
$(function(){
	...
	$('.view-videos').live("click", function(e){
		...
		if($(this).parent().hasClass('photos')){
			...
			$(this).find('.galleria').removeClass('photos-holder').addClass('videos-holder');
		}
		gimmeVideos = true;
		$.loadGalleria(currentGallery, xmlfilename, el.attr('id'));
	});
	...
	$('.view-photos').live("click", function(e){
		...
		if($(this).parent().hasClass('videos')){
			$(this).find('.galleria').removeClass('videos-holder').addClass('photos-holder');
	...
	$('.galleria iframe').attr("wmode", "opaque");
});
...
$.loadGalleria = function(cat, file, selectorid){
	...
	$.get(file, function(data){
		var imageObj = data;
		$.each(imageObj.content, function(i, image){
			categories = image.category; // this is the current images assigned categories
			for(var k in categories) {
				if(cat.toLowerCase() == categories[k].toLowerCase()) {
					videoid = image.video;
					if(!mediatrigger){
					...
					else if ((videoid !== undefined && image.isVideo === true) && gimmeVideos) {
						...
						counter++;
					} else if(image.isVideo === false && !gimmeVideos) {

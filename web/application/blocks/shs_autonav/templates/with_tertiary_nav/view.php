...
if (count($navItems) > 0) {
    echo '<ul class="treelevel treelevel-2 nav navigator-shell">'; //opens the top-level menu
    foreach ($navItems as $ni) {
    	if ($ni->cObj->getAttribute('exclude_from_primary') != true && 162 != $ni->cObj->getCollectionParentID()) {
	        ...
	        if ($ni->cID == 162) {
	            ...
	            echo '<div class="secondary-nav tertiary-nav"><ul class="treelevel treelevel-3">'; //opens a dropdown sub-menu
	            ?>
	            <div class="section-name"><a href="#">BACK</a></div>
	            <li class="parent-section"><a href="<?php echo $ni->url; ?>"><? echo $name; ?></a></li>
	            <?

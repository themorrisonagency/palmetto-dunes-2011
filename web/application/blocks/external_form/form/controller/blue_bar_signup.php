...
class BlueBarSignup extends AbstractController
{
		...
		if(!empty( $_POST )) 
		{
			...
			if(isset($response->AddUpdateSubscriberResult))
			{
				...
				if(isset($_POST['green']) && strpos($response->AddUpdateSubscriberResult, 'Error:') === false)
					...
					$this->redirect('/signup-thanks-blue');
				else
					\Log::addEntry(substr($response->AddUpdateSubscriberResult, 7) . ' - blue_bar_signup.php','Navis_Reach');
			} else
				\Log::addEntry('An error occurred while submitting form - blue_bar_signup.php!','Navis_Reach');
		}
	}
}

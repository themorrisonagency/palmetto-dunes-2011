...
echo '<ul class="mega-menu">'; //opens the top-level menu
$idx = 1;
foreach ($navItems as $ni) {
    // skip the Home page
    if (!$ni->isHome && $ni->cObj->getAttribute('exclude_from_primary') != true) {
        echo '<li class="' . $ni->classes . ' pos' . $idx . '">'; //opens a nav item
        $name = (isset($translate) && $translate == true) ? t($ni->name) : $ni->name;
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . $name . '</a>'; ?>
        <div class="secondary-nav">
            <div class="section-name"><a href="#"><? echo $name; ?></a></div>
            <?
                $stackName = 'MegaMenu: ' . htmlspecialchars_decode($name);

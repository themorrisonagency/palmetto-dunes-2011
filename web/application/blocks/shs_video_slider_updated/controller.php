<?php
namespace Application\Block\ShsVideoSliderUpdated;
...
{
    protected $btTable = 'btShsVideoSliderUpdated';
    protected $btExportTables = array('btShsVideoSliderUpdated', 'btShsVideoSliderEntriesUpdated');
    ...
    public function getSearchableContent()
    {
        ...
        $db = Loader::db();
        $v = array($this->bID);
        $q = 'select * from btShsVideoSliderEntriesUpdated where bID = ?';
        $r = $db->query($q, $v);
        foreach($r as $row) {
           $content.= $row['title'].' ';
    ...
    public function edit()
    {
        ...
        $this->requireAsset('redactor');
        $db = Loader::db();
        $query = $db->GetAll('SELECT * from btShsVideoSliderEntriesUpdated WHERE bID = ? ORDER BY sortOrder', array($this->bID));
        $this->set('rows', $query);
    }
    ...
    public function view()
    {
        ...
        $db = Loader::db();
        $r = $db->GetAll('SELECT * from btShsVideoSliderEntriesUpdated WHERE bID = ? ORDER BY sortOrder', array($this->bID));
    ...
    public function duplicate($newBID) {
        ...
        while ($row = $r->FetchRow()) {
            $db->execute('INSERT INTO btShsVideoSliderEntriesUpdated (bID, video, videoImage, videoImageMobile, promoTitle, promoDescription, promoButtonText, externalLinkURL, internalLinkCID, linkedFileID, sortOrder) values(?,?,?,?,?,?,?,?,?,?,?)',
                array(
                    $newBID,
                    $row['video'],
                    $row['videoImage'],
    ...
    {
        $db = Loader::db();
        $db->delete('btShsVideoSliderEntriesUpdated', array('bID' => $this->bID));
        parent::delete();
    }
    ...
    public function save($args)
    {
        ...
        $db = Loader::db();
        $db->execute('DELETE from btShsVideoSliderEntriesUpdated WHERE bID = ?', array($this->bID));

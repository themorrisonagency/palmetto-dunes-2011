...
(function($) {		  
		...
		return this.each(function() {
			...
			$(".date-picker", form).each(function() {
				$(this).datepicker({ 
					...
					buttonImage: "/templates/main/images/icon-cal.gif", 
		...
		return this.each(function() {
			...
			$('.submit', form).click(function() {
				...
				$('.required', form).each(function() {
					...
					if(!valid) {
						if(form.hasClass('inline')) {
							...
							var warn = $('<img />').attr('src', '/templates/smartphone/images/icon-warning.gif').addClass('validation-error');
				...
				if(_errors.length) {
					...
					if ($('#'+_errors).is('ul'))
						$('#'+_errors, form).find('input, textarea').focus();
					else
						$('#'+_errors, form).focus();
					return false;			
				}
	...
	$.fn.ajaxCalendar = function(options) {
				...
				return this.each(function() {
					$('#calendar-wrapper a').live('click',
					function(event) {
						event.preventDefault();
						overlay.fadeIn('fast');
						url = this.href.split('?');
						$.get('/events-calendar/' + url[1], {
							op: 'cal'
						},
						function(data) {
			...
			$.fn.eventPreview = function(options) {
				...
				return this.each(function() {
					$('.view-details', this).toggle(function(event) {
						...
						$.getJSON('/event-calendar/'+this.id,function(json){
	...
	$.fn.packagePreview = function(options) {
		...
		return this.each(function() {
			$('a.package-details', this).toggle(function(event) {
				event.preventDefault();
				var el = $(this);
				$.getJSON(ROOTPATH+'packages/'+this.id,function(json){
					var testnode = el.find('*').contents().filter(function() { return this.nodeType == 3 });
					testnode.length ? testnode.after(opts.hideDetails).remove() : el.html(opts.hideDetails);

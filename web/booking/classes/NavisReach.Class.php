...
class NavisReach
{
	 ...
	 */
	protected $Password = 'cmo8l7s2gdwyvjhkvurp';
	...
	/**
	 * Add or updates a crm contact
	 * @param array $data
	 */
	public function addUpdateCRMContact( $data )
        {
        
		$AddUpdateCRM = new AddUpdateCRMContact;

		$AddUpdateCRM->setAccount( $this->Account )
			->setPassword( $this->Password )
			->setResAccount( $data['res_account'] )
			->setEmailAddress( $data['email_address'] )
			->setFirstName( $data['first_name'] )
		...
		// Invoke webservice method
		$response = $client->__soapCall("AddUpdateCRMContact", array($AddUpdateCRM));

		return $response;
        }

        public function addUpdateAttributes($ParentID, $ParentType, $data) {

            $responses = array();

            if ( count( $data ) > 0 ) {
                foreach ( $data as $key => $value ) {
                    $response = $this->addUpdateAttribute($ParentID, $ParentType, $key, $value);
                    $responses[] = $response;
                }
            }
        ...
        public function addUpdateAttribute($ParentID, $ParentType, $key, $value) {
            $AddUpdateAttribute = new AddUpdateAttribute;

		$AddUpdateAttribute->setAccount( $this->Account )
			->setPassword( $this->Password )
			->setResAccount('')
            ...
            // Invoke webservice method
            $response = $client->__soapCall("AddUpdateAttribute", array($AddUpdateAttribute));
 ...
 * We are only using this request so just adding the class here.
 */
class AddUpdateCRMContact extends AddUpdateSubscriber {
}

class AddUpdateAttribute extends AddUpdateSubscriber {
...
class ReachSoapClient extends \SoapClient {
    ...
    function __doRequest($request, $location, $action, $version, $one_way = 0) {
		...
		/*
        $date = new \DateTime();         
        file_put_contents("/var/www/logs/hilton_head/" . $date->getTimestamp() . "-reach-request-test.xml", $request);
        file_put_contents("/var/www/logs/hilton_head/" . $date->getTimestamp() . "-reach-response-test.xml", $ret);
        */
